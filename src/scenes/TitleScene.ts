...
import { hasPlayerSave, loadPlayerState, resetPlayerState } from '../data/playerState';
...
export class TitleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const hasSave = hasPlayerSave();
    const startPrompt = hasSave ? 'Enter / Click: Continue    N: New Game' : 'Press Enter or Click to Start';
    const startText = this.add.text(320, 330, startPrompt, {
      color: '#f8f3df',
      fontFamily: 'monospace',
      fontSize: '20px'
    }).setOrigin(0.5);
    ...
    if (hasSave) {
      ...
      const starterLabel = state.partyCreatures[0]?.displayName ?? 'No starter selected yet';
      this.add.text(320, 360, `Continue: ${state.currentMap === 'DrSableLabScene' ? 'Dr. Sable\'s Lab' : 'Amberleaf Town'} • ${starterLabel}`, {
        align: 'center',
        color: '#f8f3df',
        fontFamily: 'monospace',
        fontSize: '13px'
