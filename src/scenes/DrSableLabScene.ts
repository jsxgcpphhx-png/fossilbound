...
const STARTER_DISPLAY_TILES: Record<StarterOption['id'], TilePosition> = {
  triceratops: { x: 6, y: 7 },
  velociraptor: { x: 10, y: 7 },
  pteranodon: { x: 14, y: 7 }
};
...
export class DrSableLabScene extends Phaser.Scene {
  ...
  private createStarterDisplays(): void {
    STARTER_OPTIONS.forEach((starter) => {
      const tile = STARTER_DISPLAY_TILES[starter.id];
